	...
	<div style="margin-bottom:50px;" v-loading="isLoading">
		<div style="color: #000;padding:15px;" class="components_cell">
			...
			<el-table :data="ActiveRecordList" border style="width: 100%" stripe :highlight-current-row="true">
				<el-table-column show-overflow-tooltip prop="activity_title" :label="$t('user.b115')" min-width="180"></el-table-column>
				...
				<el-table-column prop="status_title" :label="$t('user.b118')" width="80"></el-table-column>
				<el-table-column :label="$t('user.b87')" width="100" header-align="center" align="center">
		...
		<el-dialog width="500px" :visible.sync="payTypeDialogVisible" :title="dialogTitle">
			<div v-show="dialogType=='1'">
				<ul class="login_box">
					<li>
						<label class="inputLabel">{{$t('user.b11')}}</label>
					...
					<li>
						<label class="inputLabel">{{$t('user.b16')}}</label>
						<input style="width: 80%;" v-model="address" class="inputName">
					</li>
					<li style="text-align:center;">
						<!-- 提交按钮 -->
						<el-button style="background-color:#ffd33e;padding: 6px 24px;" @click="sendUserInfo">{{$t('user.b133')}}</el-button>
					</li>
				</ul>
			</div>
			...
			<div v-show="dialogType=='3'">
				...
				<p class="chongzhi">{{$t('user.b126')}}：{{cardInfo.card_no}}</p>
				<p class="chongzhi">{{$t('user.b127')}}：{{cardInfo.card_password}}</p>
			</div>
			<div v-show="dialogType=='4'">
				<!-- 卡密充值 -->
				<p class="chongzhi">{{$t('user.b121')}}</p>
				<p class="chongzhi">
					<span v-show="cardInfo.card_no.indexOf('：')===-1">{{$t('user.b126')}}：</span>
					{{cardInfo.card_no}}
				</p>
				<p class="chongzhi">
					<span v-show="cardInfo.card_no.indexOf('：')===-1">{{$t('user.b127')}}：</span>
