...
import { Carousel, CarouselItem, Message, Dialog } from "element-ui";
...
class User extends UserProxy {
    ...
    public version: number = 0;//用户客户端版本号
    public windowsDownloadUrl: string = "";//下载最新客户端的地址
    public downloadTipShow: boolean = false;//提示升级客户端版本弹窗
    editTitle:string='';
    //////////公共参数
    ...
    public goActivity() {
        const factory = ExtrnalFactory.getInstance().getFactory(this.appParam.platform);
        let url = JumpWebUtil.getWebHeadUrl(this.webUrl,this.appParam);
        url = url + JumpWebUtil.HTML_NAME_ACTIVITY;
        factory.jumpUrl(url);
    }
     ...
     */
    public enterToEdit() {
        if(this.newNickname == ''||this.newNickname.length==1){
            let tipMsg=this.$i18n.t('user.z47');
            this.$message({
                message: tipMsg.toString(),
                type: 'warning'
            });
            return;
        }
    ...
    public onEditAvatar() {
        if(this.appParam != null && this.appParam.platform == 1) {
            ...
            this.editTitle=this.$i18n.t('puser.r12').toString()
